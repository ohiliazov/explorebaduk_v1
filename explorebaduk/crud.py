...
from explorebaduk.database import DatabaseManager
...
from .schemas import ChallengeCreate, CreatorColor, GameCreate, GamePlayer, UserCreate
...
class FriendshipManager(DatabaseManager):
...
class BlacklistManager(DatabaseManager):
...
class GameManager(DatabaseManager):
...
class ChallengeManager(DatabaseManager):
    ...
    def get_challenges_from_user(self, user_id: int) -> List[ChallengeModel]:
        return (
            ...
            .filter(ChallengeModel.opponent_id.is_not(None))
            .all()
        )
...
class GamePlayerManager(DatabaseManager):
...
class DatabaseHandler(
    ...
    def start_game(self, challenge: ChallengeModel, opponent_id: int):
        ...
        if challenge.creator_color is CreatorColor.BLACK:
            black, white = creator, opponent
        elif challenge.creator_color is CreatorColor.WHITE:
            black, white = opponent, creator
        elif challenge.creator_color is CreatorColor.NIGIRI:
